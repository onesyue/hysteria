...
func TestGeositeRootSuffixMatch(t *testing.T) {
...
func TestGeositeRootSuffixMatchDoesNotAllocate(t *testing.T) {
	m := &geositeMatcher{Domains: []geositeDomain{{
		Type:  geositeDomainRoot,
		Value: "example.com",
	}}}
	host := HostInfo{Name: "www.example.com"}
	if allocs := testing.AllocsPerRun(1000, func() { _ = m.Match(host) }); allocs != 0 {
