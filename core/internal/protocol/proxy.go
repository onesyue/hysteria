...
func ParseUDPMessage(msg []byte) (*UDPMessage, error) {
	// This is a datagram hot path. The old implementation wrapped msg in a
	// bytes.Buffer and invoked reflection-based binary.Read four times per
	// packet. Decode the fixed header directly and let quicvarint parse its
	// variable-width field from the original slice.
	if len(msg) < 8 {
		if len(msg) == 0 {
			return nil, io.EOF
		}
	...
	addrEnd := addrStart + int(lAddr) // lAddr <= MaxMessageLength above
	if len(msg) <= addrEnd {
