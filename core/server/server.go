	...
	if r.Method == http.MethodPost && r.Host == protocol.URLHost && r.URL.Path == protocol.URLPath {
		...
		if ok {
			if tracker, ok := h.config.TrafficLogger.(ConnectionTracker); ok {
				...
				if !accepted {
					// The credential was removed after Authenticate returned but
					// before this connection could be registered. Never publish a
					// success response for that stale generation.
					h.masqHandler(w, r)
					return
				}
			}
			// Publish authenticated and authID as one mutex-protected state only
			// after the generation-aware tracker accepted registration.
...
func (h *h3sHandler) handleTCPRequest(stream *utils.QStream, authID string) {
	...
	// Publish the final state before UntraceStream removes the stats object.
	// Apart from making that lifecycle contract useful to loggers, this ensures
	// no handler mutation can race a logger that releases its final reference
	// from UntraceStream.
	defer func() {
