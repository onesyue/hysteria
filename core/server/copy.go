...
const (
	copyBufMinShift = 12 // 4 KiB: cheap for thousands of mostly-idle streams
	copyBufLevels   = 4  // 4, 8, 16, 32 KiB
	copyGrowReads   = 2  // grow after two consecutive full-buffer reads
)

// A proxy stream used to pin two 32 KiB buffers for its whole lifetime, even
// when it only exchanged a tiny request and then sat blocked.  Keep size-class
// pools instead: every direction starts at 4 KiB and reaches 32 KiB after six
// full reads, so bulk streams retain the old syscall efficiency while idle and
// interactive streams use one eighth of the in-flight buffer memory.
